window.onload = () => {
    ...
    console.log("Imported new extension script")
    // ewoq-stack-rank-box dragdrop-dropTarget
    setInterval(() => {
        let box = document.getElementsByClassName("ewoq-stack-rank-box")
        if (box.length > 0) {
            let wd = box[0].offsetWidth + "px"

            // instructions
            let hd = document.getElementsByClassName("instructions")
            if (hd.length > 0)
    ...
    // ewoq-radio-button-1
    // Yes, I understand at least generally and can rate ads for this query


    setInterval(() => {

        chrome.runtime.sendMessage({ "getAutoSeaShopping": "true" }, (response) => {
            //
            if (response == true) {

                setTimeout(() => {
                    let searchBoxs = document.getElementsByTagName("a");
                    for (let i = 0, l = searchBoxs.length; i < l; i++) {
                        let schBox = searchBoxs[i];
                        if (schBox.href.includes('https://www.google.com/search') && isNewTask == true) {
                            chrome.runtime.sendMessage({ "getTabUrlExist": schBox.href }, (response) => {
                                if (response == false) {
                                    if (isNewTask == true) {
                                        isNewTask = false;
                                        chrome.runtime.sendMessage({ "closeAnotherGgTabs": schBox.href })
                                        schBox.click();
                    ...
                    setTimeout(() => {
                        ...
                        if (allInputs.length > 0) {
                            for (let i = 0; i < allInputs.length; i++) {
                                if (allInputs[i].value == "Yes, I understand at least generally and can rate ads for this query") {
                                    let und = allInputs[i];
                                    und.click();
                                    und.onclick = () => {
                                        let nextQ = document.getElementsByClassName("mat-button")
...
// <material-button animated="true" class="finish-button _nghost-ehy-8 _ngcontent-ehy-51" tabindex="0" role="button" aria-disabled="false" elevation="1"><div class="content _ngcontent-ehy-8">Finish Reviewing</div><material-ripple aria-hidden="true" class="_ngcontent-ehy-8"><div class="__acx-ripple" style="top: -114.203px; left: -63.0781px; transform: translate(4.53906px, 4.5px) scale(0.6393);"></div><div class="__acx-ripple" style="top: -111.203px; left: -37.0781px; transform: translate(-21.4609px, 1.5px) scale(0.6393);"></div><div class="__acx-ripple" style="top: -110.203px; left: -67.0781px; transform: translate(8.53906px, 0.5px) scale(0.6393);"></div></material-ripple><div class="touch-target _ngcontent-ehy-8"></div><div class="focus-ring _ngcontent-ehy-8"></div></material-button>
// finish-button _nghost-ehy-8 _ngcontent-ehy-51
// document.getElementsByClassName("finish-button")[0]

// request-assignments _nghost-tgz-8 _ngcontent-tgz-28
// document.getElementsByClassName("request-assignments")[0]

// let startButton = document.getElementsByClassName("start-button")[0];
// let requestAssignmentsButton = document.getElementsByClassName("request-assignments")[0];

// let continueButton = document.getElementsByClassName("continue-button")[0];
// let skipButton = document.getElementsByClassName("cancel-button")[0];
