...
$(document).ready(() => {
    ...
    for (let i = 1; i <= 300; i++) {
        // console.log(convertSecondToMinute(i))
    }
    ...
    chrome.storage.local.get([
        ...
        // === Auto Sea mode
        if (items.autoSeaShopping == true) {
            $("#autoSeaSwitch").attr("checked", true);
        }
        ...
        let autoSubmitAfterSelect = document.getElementById("autoSubmitAfterSelect");
        let inHtmlAutoSubmitAfterSelect = "";
        for (let i = 1; i <= 300; i++) {
            inHtmlAutoSubmitAfterSelect += '<option value="' + i + '">' + convertSecondToMinute(i) + ' s</option>';

        }
        autoSubmitAfterSelect.innerHTML = inHtmlAutoSubmitAfterSelect;
        //
        ...
        $("#inputShowAutoSubmitWhileRemain").prop("max", time);
    ...
    //..... Auto Sea x Shopping mode
    $(document).on("click", "#autoSeaSwitch", () => {
        if ($("#autoSeaSwitch").is(":checked")) {
            chrome.storage.local.set({ "autoSeaShopping": true })
        }
        else {
    ...
    $(document).on("change", "#autoSubmitAfterSelect", () => {
        ...
        chrome.storage.local.get(["showAutoSubmitWhileRemaining"], (items) => {
            ...
            if (true) {
                ...
                $("#inputShowAutoSubmitWhileRemain").val((time).toString());
                $("#inputShowAutoSubmitWhileRemain").prop("max", time);
            }
        })
    ...
    $("#inputShowAutoSubmitWhileRemain").on("change", () => {
        ...
        chrome.storage.local.get(["autoSubmitAfter"], (items) => {
            time = items.autoSubmitAfter;
            $("#inputShowAutoSubmitWhileRemain").val((time).toString())
            if (time > (items.autoSubmitAfter)) {
